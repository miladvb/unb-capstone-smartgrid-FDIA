...
avg_weights = None
def federated_averaging(local_weights, num_clients):
    global avg_weights
    if avg_weights is None:
        ...
        for key in avg_weights.keys():
            avg_weights[key] = (avg_weights[key] * (num_clients - 1) + local_weights[key]) / num_clients
    return avg_weights
...
for round in range(rounds):
    ...
    for client in range(num_clients):
        ...
        local_w, local_loss = local_train(client_model[client], train_loader_client, client_optimizer, criterion, local_epochs)

        avg_weights = federated_averaging(local_w, num_clients)
